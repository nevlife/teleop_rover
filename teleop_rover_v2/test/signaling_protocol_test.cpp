...
using teleop_rover_v2::IceCandidate;
using teleop_rover_v2::IncomingMessage;
using teleop_rover_v2::ProtocolError;
using teleop_rover_v2::build_answer;
using teleop_rover_v2::build_hello;
using teleop_rover_v2::build_ice;
using teleop_rover_v2::build_offer;
using teleop_rover_v2::parse_message;
...
TEST(SignalingProtocol, ParsesHelloAckWithTurnCredentials)
{
  const auto message = parse_message(R"({
    ...
    "robot_id": "rover-01",
    "turn": {"urls": ["turn:203.250.33.64:13437?transport=udp"],
  ...
  EXPECT_EQ(message.hello_ack.turn->urls[0], "turn:203.250.33.64:13437?transport=udp");
