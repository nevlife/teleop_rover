...
| Package | Role |
|---|---|
| [`teleop_rover_v2/`](teleop_rover_v2/) | Safety boundary (`ControlGuard`) and the native WebRTC media endpoint |
...
cd ~/ros2_ws
colcon build --symlink-install --packages-select teleop_rover_v2 teleop_rover_msgs
source install/setup.bash
```

`teleop_rover_v2` needs only `ament_cmake`; `rosdep` is not required for it.
...
```bash
colcon test --packages-select teleop_rover_v2
...
```bash
ros2 run teleop_rover_v2 teleop-media-probe
```
