...
using teleop_rover::AcceptedMotion;
using teleop_rover::ControlGuard;
using teleop_rover::EncoderChoice;
using teleop_rover::FakeVehicle;
...
using teleop_rover::TurnCredentials;
...
void on_offer_created(GstPromise * promise, gpointer user_data)
{
  ...
  send_text(rover, teleop_rover::build_offer(text));
  g_free(text);
...
bool start_pipeline(Rover * rover)
{
  ...
  std::string description = teleop_rover::build_send_pipeline(
...
void handle_message(Rover * rover, const std::string & text)
{
  ...
  try {
    message = teleop_rover::parse_message(text);
  } catch (const teleop_rover::ProtocolError & error) {
...
void on_connected(GObject * source, GAsyncResult * result, gpointer user_data)
{
  ...
  send_text(rover, teleop_rover::build_hello(rover->robot_id));
}
...
int main(int argc, char ** argv)
{
  ...
  try {
    config_path =
      ament_index_cpp::get_package_share_directory("teleop_rover") +
  ...
  Rover rover;
  try {
    rover.config = teleop_rover::load_media_config(config_path);
  } catch (const teleop_rover::ConfigError & error) {
    std::fprintf(stderr, "teleop-rover: %s\n", error.what());
    return 2;
  }
  ...
  const auto choice = teleop_rover::select_encoder(rover.config.video.codec_preference);
  if (!choice) {
    std::fprintf(
      stderr,
